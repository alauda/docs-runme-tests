...
# OpenSearch 自动安装（默认开启；前提：业务集群至少 3 个节点、各节点有空闲磁盘）
# 开启且下方 TopoLVM 两个插件包地址齐全时，OpenSearch 安装测试的步骤 0 会自动安装 TopoLVM + OpenSearch，
# 并用实际安装结果覆盖 TRACING_OPENSEARCH_*（无需手动配置）；条件不满足时降级用手动配置。
# 三个插件包都由步骤 0 按需下载并 violet 上架到业务集群，已上架的直接跳过（不重复下载）；
# 地址留空即 verify-only，要求该插件已在业务集群预上架，否则报错退出。
...
export PKG_OPENSEARCH_OPERATOR_URL=xxx    # Alauda support for OpenSearch 包（自动下载上架）
# TopolvmCluster 使用的节点空闲磁盘设备（可选，默认 /dev/vdb）
export TRACING_TOPOLVM_DEVICE=/dev/vdb
# OpenSearch / Dashboards 版本（可选，默认 3.7.0，需与 opensearch-operator 插件包内置镜像 tag 对应）
# export TRACING_OPENSEARCH_VERSION=3.7.0
# export TRACING_OPENSEARCH_DASHBOARDS_VERSION=3.7.0
# opensearch-operator 安装的命名空间与订阅 channel（可选，默认 opensearch-operator + 包的
# defaultChannel；集群上已有该 operator 时自动沿用它所在的命名空间）
# export TRACING_OPENSEARCH_OPERATOR_NS=opensearch-operator
# export TRACING_OPENSEARCH_OPERATOR_CHANNEL=alpha
...
# /services、/operations、/trace-summaries，断言窗口内至少有 2 条调用链；不够就整轮重试。
# 查两次：先按服务名 jaeger（Jaeger 自身的调用链，跳过 telemetrygen 时也一定有），
# 真跑过 telemetrygen 时再按服务名 telemetrygen 查一次（业务侧 span 的完整通路）。
# 默认服务名可用 TRACING_VERIFY_TRACE_SERVICE 覆盖，条数门槛用 TRACING_VERIFY_TRACE_MIN_COUNT
# （默认 2）；重试与窗口见 projects/tracing/trace-query.sh。
