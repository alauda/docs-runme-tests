...
# 先按服务名 jaeger 查（Jaeger 自身的调用链，跳过 telemetrygen 时也一定有），真跑过
# telemetrygen 时再按服务名 telemetrygen 查一次（业务侧 span 的完整通路）；SPM 章节重新
# 部署 telemetrygen 之后还会再验一次（那次 patch 重启过 Jaeger 并改了 Collector 路由）。
