...
#   - 其余分支（特性分支）                          -> <净化后的分支名>-<短 commit>，仅此一个
#
...
# 特性分支**不产出** latest / release-<x> 这类浮动 tag，避免污染 dailybuild 在用的镜像。
#
...
if [ -n "$acp_version" ]; then
    printf 'release-%s,%s-%s\n' "$acp_version" "$(_sanitize_tag "$branch")" "$short"
    exit 0
fi

# release-mesh-* 形状却没登记：这是漏登记，不能悄悄降级成特性分支 tag，
# 否则发版镜像不会带 release-<ACP大版本> 这个 dailybuild 真正引用的浮动 tag。
case "$branch" in
    release-mesh-*)
        printf '错误: 发版分支 %s 未在 %s 中登记对应的 ACP 大版本\n' "$branch" "$MATRIX" >&2
        exit 1
        ;;
esac
